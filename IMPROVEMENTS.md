...
### 4.7 Realtime Session Statistics Panel

**Current state:** During a session, the stats row under the graph shows instantaneous metrics (Shamatha, Distraction, Sinking, NS Attn, NS Med). The aggregate stats shown in the end-of-session summary (Duration, Avg Shamatha, Avg Meditation, Time Above Threshold, Time Shamatha ≥90, best streak) are only visible *after* stopping.

**Problem:** The meditator can't see their progress accumulating. Knowing "I've been above threshold for 4 of 10 minutes so far" reinforces the practice in the moment. Post-session reveal is useful for review but loses the in-session motivation.

**Proposal:**
- Extend the stats row (or add a second thin row near the existing realtime metrics) with running aggregates, updated every tick:
  - Duration (session elapsed)
  - Avg Shamatha (cumulative mean)
  - Time Above Threshold
  - Time Shamatha ≥90
  - Current streak (consecutive ticks above threshold)
- Same numbers as the end-of-session card — reuse the same computation so there's one source of truth.
- Optional toggle: "Show live stats" on/off for users who want a minimal view (ties in with Zen Mode 2.3).

**Why it matters:** Converts the in-session screen from "here's your brainwaves" into "here's your practice". Closes the feedback loop between effort and visible progress.

---

### 4.8 Scrollable Live Session Layout on Narrow / Rotated Screens — **Implemented 2026-04-19.**

**Current state:** Live Session screen packs header, view toggle, graph, stats, preset row, and controls into a fixed vertical layout. On landscape rotation or small phones, the graph gets squeezed and stats/controls crowd together.

**Problem:** The graph is the thing users actually watch during meditation, but it loses vertical real estate to fixed-height chrome when the screen is short. In landscape it becomes almost unusable.

**Proposal:**
- Wrap the Live Session content in a `ScrollView` with the graph set to fill the viewport (size_hint_y=1, min_height = full screen height minus header).
- Stats row, preset row, and Start/Pause/Stop/Mark controls sit *below* the graph and become reachable by scrolling down.
- On portrait desktops / tall screens, nothing changes — everything fits without scrolling.
- Consider a short persistent bar for Start/Pause/Stop at the bottom that stays visible regardless of scroll, so the primary actions are always one tap away.

**Why it matters:** Keeps the data the user cares about (the graph) dominant at any aspect ratio, without hiding controls.

---

### 4.9 Device Selector on Live Session

**Current state:** The device label (top-left of Live Session, e.g. `[Mock EEG]` or `[MindWave Mobile]`) is read-only. To switch between mock and real, or to pick a different paired device, users must navigate to Settings → Device.

**Problem:** Switching device is a common pre-session action (e.g., "let me quickly run a mock session to test a formula"). Two taps of navigation vs. one tap on the label already in view is friction.

**Proposal:**
- Make the device label tappable. Tapping opens a small popup/sheet with:
  - Toggle: Real Device / Mock Device
  - If Real: list of paired MindWave devices (populated from existing scan); tap one to select. Active selection indicated.
  - Button: "Scan for devices" — triggers the same scan action as in Settings.
- Selection is persisted via the same user settings path as the Settings screen (`use_mock`, `bt_device_address`, `bt_device_name`) — one source of truth.
- Disabled while a session is running (can't swap device mid-session).

**Why it matters:** Quick mock↔real toggle is valuable for formula research and debugging, and it's the most natural place on the screen for device control.

---

### 4.10 Compact Power-Line Noise Indicator

**Current state:** The power-line noise detector shows a text warning when 50/60Hz interference is detected. The message is verbose and on Android (narrow screen widths) it truncates.

**Problem:** A truncated warning is worse than no warning — users see an incomplete sentence and don't know what action to take. Vertical real estate is scarce on phones.

**Proposal:**
- Replace the text banner with a compact icon + short label, e.g. `⚡ 50Hz` or `⚡ AC noise` with a distinctive color (amber / red).
- Tap/long-press the indicator to open a tooltip or popup with the full explanation and mitigation tips (move away from outlets, enable notch filter per 6.6, etc.).
- Fits within a single row height; same placement across platforms.
- Related to 6.6 (Power-Line Noise Mitigation) — the indicator and filter toggle could live together.

**Why it matters:** Keeps the warning visible and legible on the narrowest supported screen without sacrificing information — the detail is one tap away.
...
### 6.7 Crash Report Dialog with Copyable Traceback — **Implemented 2026-04-19.**

**Current state:** On any unhandled exception in the main loop, Kivy logs to stderr / logcat but the user sees either a silent app crash (Android) or a terminal stack trace (desktop). There's no in-app path from "the app crashed" to "I filed a useful bug report".

**Problem:** Users — especially non-technical meditators — have no way to report what went wrong. Even developer users have to fish logs out of `~/.kivy/logs` or `adb logcat`. Crashes without telemetry are invisible.

**Proposal:**
- Install a global `sys.excepthook` (and `threading.excepthook` for background EEG/audio threads) that catches any unhandled exception.
- On exception, show a modal dialog containing:
  - A friendly "The app hit an unexpected error" heading.
  - **System info block:** app version, OS + version, Python version, Kivy version, device name (MindWave / Mock), session state if any.
  - **Full traceback** in a scrollable, monospaced text field.
  - Buttons: **Copy to clipboard**, **Save to file**, **Open GitHub Issues**, **Dismiss**.
- Copy button copies the full system info + traceback as a pre-formatted markdown code block, ready to paste into a new GitHub issue.
- Save button writes to `app_storage_path()/crash_<timestamp>.txt`.
- "Open GitHub Issues" opens the project's issues URL in the system browser.
- Session data is auto-saved to the DB before the dialog is shown (ties in with 6.4 Auto-Save on Unexpected Exit).
- Don't swallow exceptions that occur *during* dialog display — fall back to logging in that case.

**Why it matters:** Turns silent crashes into actionable bug reports. For researchers running custom formulas, this is how subtle parser or evaluator bugs actually surface. Lowers the bar to contribute diagnostic info from "learn adb" to "tap Copy, paste into issue".
